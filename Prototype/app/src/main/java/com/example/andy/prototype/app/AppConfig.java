package com.example.andy.prototype.app;

public class AppConfig {

    // Server user login url
    public static String URL_LOGIN = "http://10.0.2.2:8080/SpringMVCHibernate/login";

    // Server user register url
    public static String URL_REGISTER = "http://10.0.2.2:8080/SpringMVCHibernate/register";

    // Server user register url
    public static String URL_TWITTER_API = "https://api.twitter.com/1.1/search/tweets.json";

    // Server user register url
    public static String URL_CREATE_COMPANY = "http://10.0.2.2:8080/SpringMVCHibernate/company/create";
}

